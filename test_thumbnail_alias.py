#!/usr/bin/env python3
"""
Test using HFS path format (alias string) instead of POSIX file.
"""
...
# NEW APPROACH: Get HFS path using AppleScript itself
get_hfs_path_script = f'''
set posixPath to "{jsx_file}"
set hfsPath to POSIX file posixPath as text
return hfsPath
'''

print("Converting POSIX path to HFS path...")
result = subprocess.run(
    ['osascript', '-e', get_hfs_path_script],
    capture_output=True,
    text=True
)
...
if result.returncode == 0:
    ...
    print(f"HFS Path: {hfs_path}")
    print()
    
    # Now use the HFS path with do javascript
    applescript = f'''tell application "Adobe Photoshop 2025"
    activate
    do javascript alias "{hfs_path}"
end tell'''
    ...
    print("❌ Failed to convert POSIX path to HFS path")
    print(result.stderr)
