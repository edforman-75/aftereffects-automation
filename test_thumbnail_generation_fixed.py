#!/usr/bin/env python3
"""
Fixed isolated test script for Photoshop thumbnail generation.
"""
...
print("Generated ExtendScript JSX")
print()
...
# FIX: Read the JSX content and pass it directly to Photoshop instead of using a file path
with open(jsx_file, 'r') as f:
    jsx_content = f.read()

# Escape the JSX content for AppleScript
jsx_escaped = jsx_content.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n').replace('\r', '')

# Build AppleScript - pass JSX content directly instead of file path
applescript = f'''
...
print("Generated AppleScript (using inline script instead of file path)")
print("=" * 60)
